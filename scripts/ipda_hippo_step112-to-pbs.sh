...
Usage: bash ipda_hippo_step112-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col2:
                            /path/from/working/dir/to/counts.tsv
                            Note: for example, users could simply
                            grep "sample" hippo111_asl_SamTools_DATE/*.counts.tsv > counts.tsv;
                            grep -v "sample" hippo111_asl_SamTools_DATE/*.counts.tsv >> counts.tsv

                            Col3:
                            /path/from/working/dir/to/HIPPO/scripts/ipda_hippo_step112.r
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_hippo_step112-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hippo_step112-to-pbs_${thislogdate}.txt
...
outpath_hippo112_plots="hippo112_asl-plots_R_${thislogdate}"

## Create output directories
mkdir -p ${outpath_hippo112_plots}
...
date
echo "## Executing bash ipda_hippo_step112-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_hippo112_plots}"
...
cut -f1 ${input} | sort | uniq | while read stem; do echo "#................................................" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do echo 'echo "## Plot stem at" ; date ; echo' >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do counts=`grep "${stem}" ${input} | cut -f2 | sort | uniq`; rscript=`grep "${stem}" ${input} | cut -f3 | sort | uniq`; echo "Rscript ${rscript} --inputc ${counts} --outdir ${outpath_hippo112_plots}/ --outstem \"${stem}\"" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HIPPO step 112 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_hippo112_plots},'"${outpath_hippo112_plots}"',g' "$logfile"
