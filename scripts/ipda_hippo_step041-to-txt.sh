...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col8:
                            ligation-site1,ligation-site2,ligation-siteN
...
cut -f1 ${input} | sort | uniq | while read path_folder; do file=`echo ${path_folder} | rev | cut -d"/" -f2 | rev` ; ligation=`grep "${path_folder}" ${input} | cut -f8 | sort | uniq`; echo "LIGATION_SITE = ${ligation}" >> config-hicpro_${file}_${thislogdate}.txt; done
...
cut -f1 ${input} | sort | uniq | while read path_folder; do file=`echo ${path_folder} | rev | cut -d"/" -f2 | rev` ; echo "#######################################################################" >> config-hicpro_${file}_${thislogdate}.txt; done
cut -f1 ${input} | sort | uniq | while read path_folder; do file=`echo ${path_folder} | rev | cut -d"/" -f2 | rev` ; echo "" >> config-hicpro_${file}_${thislogdate}.txt; done
cut -f1 ${input} | sort | uniq | while read path_folder; do file=`echo ${path_folder} | rev | cut -d"/" -f2 | rev` ; echo "BIN_SIZE = 2000 5000 20000 40000 150000 500000" >> config-hicpro_${file}_${thislogdate}.txt; done
