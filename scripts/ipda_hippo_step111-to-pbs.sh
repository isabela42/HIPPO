...
cut -f1 ${input} | sort | uniq | while read stem; do grep -F "${stem}" ${input} | cut -f9 | sort | uniq | while read window; do bin=`grep -F "${stem}" ${input} | cut -f4 | sort | uniq`; chr1=`grep -F "${stem}" ${input} | cut -f5 | sort | uniq`; start1=`grep -F "${stem}" ${input} | cut -f6 | sort | uniq`; chr2=`grep -F "${stem}" ${input} | cut -f7 | sort | uniq`; start2=`grep -F "${stem}" ${input} | cut -f8 | sort | uniq`; echo "chr1=${chr1}; startr1=${start1}; bin=${bin}; startw1=\$(( (startr1-1)/bin*bin + 1 )); endw1=\$(( startw1 + bin - 1 )); ${window}_coordw1=\`echo "\${chr1}:\${startw1}-\${endw1}"\`\tchr2=${chr2}; startr2=${start2}; bin=${bin}; startw2=\$(( (startr2-1)/bin*bin + 1 )); endw2=\$(( startw2 + bin - 1 )); ${window}_coordw2=\`echo "\${chr2}:\${startw2}-\${endw2}"\`" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read stem; do grep -F "${stem}" ${input} | cut -f9 | sort | uniq | while read window; do snp=`grep -F "${stem}" ${input} | cut -f10 | sort | uniq`; ref=`grep -F "${stem}" ${input} | cut -f11 | sort | uniq`; echo "samtools mpileup -f ${ref} -r ${snp} -o ${outpath_hippo111_samtools}/${stem}.${window}.mpileup ${outpath_hippo111_samtools}/${stem}.${window}.bam" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done done
