...
cut -f1 ${input} | sort | uniq | while read stem; do flag_merge=`grep -F "${stem}" ${input} | cut -f3 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then echo \"inputbam=\"${outpath_hippo111_samtools}/${stem}.merged.bam\"\" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; elif [[ $flag_merge = "no" ]]; then echo nonmergebam=`grep -F "${stem}" ${input} | cut -f2`; echo \"inputbam=\"${nonmergebam}\"\" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; else echo "Unknown flag provided. Please provide either yes or no and try again" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; fi; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read stem; do grep -F "${stem}" ${input} | cut -f9 | sort | uniq | while read window; do bin=`grep -F "${stem}" ${input} | cut -f4 | sort | uniq`; chr1=`grep -F "${stem}.*${window}" ${input} | cut -f5 | sort | uniq`; start1=`grep -F "${stem}.*${window}" ${input} | cut -f6 | sort | uniq`; chr2=`grep -F "${stem}.*${window}" ${input} | cut -f7 | sort | uniq`; start2=`grep -F "${stem}.*${window}" ${input} | cut -f8 | sort | uniq`; echo "chr1=${chr1}; startr1=${start1}; bin=${bin}; startw1=\$(( (startr1-1)/bin*bin + 1 )); endw1=\$(( startw1 + bin - 1 )); ${window}_coordw1=\`echo "\${chr1}:\${startw1}-\${endw1}"\`\tchr2=${chr2}; startr2=${start2}; bin=${bin}; startw2=\$(( (startr2-1)/bin*bin + 1 )); endw2=\$(( startw2 + bin - 1 )); ${window}_coordw2=\`echo "\${chr2}:\${startw2}-\${endw2}"\`" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read stem; do grep -F "${stem}" ${input} | cut -f9 | sort | uniq | while read window; do echo "samtools view -@${ncpus} -b -h ${outpath_hippo111_samtools}/${stem}.sorted.bam \${${window}_coordw1} \${${window}_coordw2} > ${outpath_hippo111_samtools}/${stem}.${window}.bam" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read stem; do grep -F "${stem}" ${input} | cut -f9 | sort | uniq | while read window; do echo "samtools index -@${ncpus} ${outpath_hippo111_samtools}/${stem}.${window}.bam" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read stem; do grep -F "${stem}" ${input} | cut -f9 | sort | uniq | while read window; do snp=`grep -F "${stem}.*${window}" ${input} | cut -f10 | sort | uniq`; ref=`grep -F "${stem}" ${input} | cut -f11 | sort | uniq`; echo "samtools mpileup -f ${ref} -r ${snp} -o ${outpath_hippo111_samtools}/${stem}.${window}.mpileup ${outpath_hippo111_samtools}/${stem}.${window}.bam" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done done
cut -f1 ${input} | sort | uniq | while read stem; do echo -e \"sample\tref\talt\ttotal\" >> ${outpath_hippo111_samtools}/${stem}.counts; grep -F "${stem}" ${input} | cut -f9 | sort | uniq | while read window; do echo "ref=\`cut -f5 ${outpath_hippo111_samtools}/${stem}.${window}.mpileup.txt | awk '{print gsub(/[.,]/, "")}'\`; alt=\`cut -f5 ${outpath_hippo111_samtools}/${stem}.${window}.mpileup | awk '{print gsub(/[ACGTNacgtn]/, "")}'\`; total=$((ref + alt)) echo -e \"${stem}_${window}\t\${ref}\t\${alt}\t\${total}\" >> ${outpath_hippo111_samtools}/${stem}.counts" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done done
...
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n20 ${pbs} | head -n1)" ; echo "#                       $(tail -n19 ${pbs} | head -n1)"; echo "#                       $(tail -n17 ${pbs} | head -n1)"; echo "#                       $(tail -n14 ${pbs} | head -n1)"; echo "#                       $(tail -n11 ${pbs} | head -n1)"; echo "#                       $(tail -n8 ${pbs} | head -n1)"; echo "#                       $(tail -n5 ${pbs} | head -n1)"; echo "#                       $(tail -n2 ${pbs} | head -n1)"; echo "#                       $(tail -n1 ${pbs})"; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done
