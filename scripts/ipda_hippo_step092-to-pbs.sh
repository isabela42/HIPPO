...
cut -f1 ${input} | sort | uniq | while read stem; do echo "files <- list.files(interactiondir, pattern = \"\\\\.pgl$\")" >> ${pbs_stem}_${stem}_${thislogdate}.r; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "samples <- unique(sapply(strsplit(gsub(\"^risk-variants_\", \"\", files), \"\\\\.pgl$\"), \`[\`, 1)) # if you had a dir per sample: dir(interactiondir)" >> ${pbs_stem}_${stem}_${thislogdate}.r; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.r; done
...
cut -f1 ${input} | sort | uniq | while read stem; do echo "  j <- paste0(interactiondir, \"/\",  ifelse(file.exists(paste0(interactiondir, \"/risk-variants_\", i, \".pgl\")), paste0(\"risk-variants_\", i), i), \".pgl\")" >> ${pbs_stem}_${stem}_${thislogdate}.r; done
