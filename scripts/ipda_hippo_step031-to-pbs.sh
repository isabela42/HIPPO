...
Usage: bash ipda_hippo_step031-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources baseline: -m 1 -c 1 -w "00:30:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col2:
                            path/to/save/output
                            File will be named as chrom.genome-stem.sizes
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_hippo_step031-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hippo_step031-to-pbs_${thislogdate}.txt
...
date
echo "## Executing bash ipda_hippo_step031-to-pbs.sh"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Get chromosome sizes at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; output=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; echo "samtools faidx ${path_file} ; cut -f1,2 ${path_file}.fai > ${output}/chrom_${file}.sizes" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HIPPO step 031 jobs) at
qstat -u "$user"
