...
Description: Write and submit PBS jobs for step 021 of HIPPO
(HiChIP Integration Pipeline for PBS Operations).

Usage: bash ipda_hippo_step021-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/genome.fasta

                            Col2:
                            restriction enzymes separated by space
                            Examples:
                            -> hindiii dpnii bglii
                            HindIII, DpnII and BglII are encoded within the script and are therefore recognized if specified to the program
                            -> ^GATC G^ANTC
                            Re. the Arima kit

                            Col3:
                            path/to/save/output
                            File will be named as genome-stem_enzymes.bed
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_hippo_step021-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hippo_step021-to-pbs_${thislogdate}.txt
...
digest_genome="/software/HiC-Pro/HiC-Pro_2.11.4/bin/utils/digest_genome.py"
...
date
echo "## Executing bash ipda_hippo_step021-to-pbs.sh"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_hicpro}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_python}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Generate the list of restriction fragments after digestion at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; enzymes=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; output=`grep "${path_file}" ${input} | cut -f3 | sort | uniq`; enzymes_out=`echo ${enzymes} | tr ' ' '_'`; echo "python ${digest_genome} -r ${enzymes} -o ${output}/${file}_${enzymes_out}.bed ${path_file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HIPPO step 021 jobs) at
qstat -u "$user"
...
sed -i 's,${digest_genome},'"${digest_genome}"',g' "$logfile"
